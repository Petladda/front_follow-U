...
export default function () {
  ...
  const loadMySubject = async () => {
    let result = await client.get(`/api/user/subject`).then((r) => {
      console.log(r.data);
      setMySubject(r.data);
    });
  };
    ...
    <main className=" px-4 my-8  h-screen ">
        ...
        {mysubject.map((e) => {
          return (
            <div
              key={e.id}
              {...e}
              className="border mb-2 rounded-lg my-2 px-5 w-full h-11 flex flex-row justify-between"
            >
              <div
                onClick={() => router.push(`subject/${e.id}`)}
                className="my-2 cursor-pointer"
              >
                {" "}
                ID: {e.id} วิชา {e.subject_name}{" "}
              </div>
              <div className="flex flex-row justify-end ">
                <svg
                  onClick={() => handleSelectSubject(e)}
      ...
      {openmodalsubject && (
        <ModalUpdateSubject
          ...
          closesubject={setOpenModalSubject}
        />
      )}
    </main>
  );
